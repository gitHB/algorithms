...
blockchain = Blockchain()  # Создание экземпляра блокчейна
new_data = ["Транзакция1", "Транзакция2"]  # Данные для нового блока
new_block = Block(data=new_data)  # Создание нового блока
blockchain.addBlock(new_block)  # Добавление нового блока в блокчейн
is_valid = blockchain.isValid()  # Проверка валидности блокчейна
print("Блокчейн валиден:", is_valid)
print(blockchain)  # Вывод всей цепочки блоков